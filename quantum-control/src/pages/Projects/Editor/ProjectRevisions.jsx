...
export default function ProjectRevisions({ data, refreshProject }) {
  ...
  const loadRevisions = async () => {
    try {
      setLoading(true);
      const revs = await projectService.getProjectRevisions(data.id);
      setRevisions(revs);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleCreateSnapshot = async (e) => {
    ...
    try {
      setCreating(true);
      await projectService.createProjectRevision(data.id, userProfile.id, label);
      setLabel('');
      await loadRevisions();
    } catch (err) {
      console.error(err);
  ...
  const handleRestore = async (revisionId) => {
    ...
    try {
      setRestoring(true);
      await projectService.restoreProjectRevision(data.id, revisionId, userProfile.id);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
      ...
      <div>
        ...
        <p style={{ color: 'var(--admin-placeholder)', marginBottom: '1.5rem' }}>
          Manually save a named version of this project. Useful before making major changes or publishing.
        </p>
        ...
        <form onSubmit={handleCreateSnapshot} style={{ display: 'flex', gap: '1rem', background: 'rgba(255,255,255,0.02)', padding: '1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <input 
            type="text"
            placeholder="e.g. 'Before rewriting introduction'"
            value={label}

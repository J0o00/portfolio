...
export default function ProjectTags({ data, update }) {
  ...
  const handleCreateTag = async (e) => {
    ...
    try {
      setCreating(true);
      const newTag = await projectService.createTag(newTagName);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', margin: '0 0 1rem 0' }}>
          <Tag size={20} /> Project Tags
        </h3>
        <p style={{ color: 'var(--admin-placeholder)', marginBottom: '1.5rem' }}>
          Categorize your project to help visitors find related content.
        </p>
      </div>

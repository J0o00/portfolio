...
export default function ExperienceTags({ data, update, experienceId }) {
  ...
  const loadTags = async () => {
    try {
      setLoading(true);
      const tags = await experienceService.getTags();
      setAllTags(tags);
    } catch (err) {
  ...
  const handleAddTag = async (tag) => {
    try {
      // In experience we use live tags since we're in the editor
      if (experienceId) {
        await experienceService.addTagToExperience(experienceId, tag.id);
      }
  ...
  const handleRemoveTag = async (tagIdToRemove) => {
    try {
      if (experienceId) {
        await experienceService.removeTagFromExperience(experienceId, tagIdToRemove);
      }
  ...
  const handleCreateTag = async (e) => {
    ...
    try {
      setCreating(true);
      const newTag = await experienceService.createTag(newTagName);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', margin: '0 0 1rem 0' }}>
          <Tag size={20} /> Experience Tags
        </h3>
        <p style={{ color: 'var(--admin-placeholder)', marginBottom: '1.5rem' }}>
          Categorize your experience to help visitors find related background details.
        </p>
      </div>
